...
{
    public enum Skill { Fireball, Spike, Hook }
    ...
    [SerializeField] private float _hookArrivalDistance = 1.5f;

    [Header("Spikes")]
    [SerializeField] private float _spikeLifetime = 2f;
    [SerializeField] private int _maxSpikes = 5;

    private Skill _activeSkill = Skill.Fireball;
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.C)) { _activeSkill = Skill.Fireball; CastFireball(); }
        if (Input.GetKeyDown(KeyCode.V)) { _activeSkill = Skill.Spike; SpawnSpike(); }
        if (Input.GetKeyDown(KeyCode.B)) { _activeSkill = Skill.Hook; Hook(); }
    }
